#!/usr/bin/env python3
"""
Preprocess Language Reactor CSV exports into a simpler format for cleanup.

This script:
1. Parses the Language Reactor export
...
def parse_language_reactor_export(filepath: Path) -> tuple[list[dict], str | None]:
    """Parse a Language Reactor export file.
    ...
    with open(filepath, encoding="utf-8") as f:
        for line in f:
            ...
            if entry["lemma"] and entry["translation"]:
...
def write_preprocessed_csv(entries: list[dict], output_path: Path):
    ...
    with open(output_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Word", "Translation", "Context (RU)", "Context (EN)", "POS"])
...
def main():
    parser = argparse.ArgumentParser(
        description="Preprocess Language Reactor CSV exports."
    )
    parser.add_argument(
        "input_file",
        type=Path,
        help="Language Reactor export file",
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
